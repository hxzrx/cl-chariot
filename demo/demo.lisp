...
;;;; 运行(仓库根目录):
;;;;   demo/run.sh            —— 全部示例
;;;;   或分别调用:(clh-demo:example-chat) 等
;;;;
;;;; 环境变量:
;;;;   CLH_PROVIDER   厂商预设名,默认 deepseek
;;;;   CLH_MODEL      模型名,默认取厂商预设
;;;;   CLH_API_KEY    API Key(未设时回退到对应厂商的标准环境变量)

(defpackage :clh-demo
  (:documentation "CL-Harness 完整示例包。")
  (:use :cl)
  (:import-from :clh-llm
...
(defun demo-provider-spec ()
  ...
  (let ((provider-name (intern (string-upcase (or (uiop:getenv "CLH_PROVIDER") "deepseek"))
                               :keyword)))
    (values provider-name (uiop:getenv "CLH_MODEL"))))
...
CLI 内置的 clh-cli:make-event-printer 更完整;这里给出精简版供嵌入方参考。"
  (lambda (event)
  ...
  (let ((provider (make-demo-provider)))
    ...
    (let ((chunks 0))
      (multiple-value-bind (message usage finish)
          (clh:chat provider
                    (list (clh:make-user-message "用一句话说明什么是 Common Lisp 的条件系统"))
                    :stream t
...
(defun arithmetic-ast (tokens)
  ...
  (labels ((primary (ts)
             (cond ((null ts)
                    (error 'clh-tools:tool-error :message "表达式不完整"))
                   ((numberp (first ts))
                   ...
                   ((and (characterp (first ts)) (char= (first ts) #\())
                    (multiple-value-bind (v rest) (expr (rest ts) 1)
                      (unless (and rest (characterp (first rest)) (char= (first rest) #\)))
                        (error 'clh-tools:tool-error :message "缺少右括号"))
                      (values v (rest rest))))
                   (t (error 'clh-tools:tool-error :message "意外的记号"))))
    ...
    (multiple-value-bind (value rest) (expr tokens 1)
      (when rest
        (error 'clh-tools:tool-error :message "表达式存在多余记号"))
      value)))
...
(defun evaluate-arithmetic (expression-string)
  ...
  (let ((ast (arithmetic-ast (tokenize-arithmetic expression-string))))
    (let ((value (handler-case (eval ast)
                   ...
                   (division-by-zero ()
                     (error 'clh-tools:tool-error :message "除数为零")))))
...
(defun make-calculator-tool ()
  ...
  (make-tool
   ...
   :handler
   (lambda (args)
     (let ((expr (clh-json:jref args "expression" "")))
       (if (clh-util:string-blank-p expr)
           (error 'clh-tools:tool-error :message "缺少表达式")
...
(defun example-tools ()
  ...
  (let ((agent (clh:make-agent
    ...
    (let ((result (clh:run agent "请计算 17 * 23 + 4 的结果,并告诉我今天几号。回答保持简短。")))
      (format t "最终回答:~A~%" (or (clh:result-text result) "(空)")))))
...
请设置环境变量后重试,例如:
  CLH_PROVIDER=deepseek DEEPSEEK_API_KEY=sk-... demo/run.sh~%"
              e))))
