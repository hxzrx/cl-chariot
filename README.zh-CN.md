...
`:permission-denied` `:turn-start/end` `:cancel` `:provider-switch` `:run-end` 等)。
...
```
cl-chariot/
├── cl-chariot.asd          # 全部系统定义(base/llm/tools/agent/mcp/伞形/cli/test/demo)
├── src/                    # 源码(全部带中文文档注释)
