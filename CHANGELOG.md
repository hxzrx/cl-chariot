...
无既有行为变更,新增均为增量(新文件/新导出/文档)。
版本政策:不切 1.0,待真实使用反馈后再议。

### 文档
- 新增嵌入指南(docs/embedding.md):生命周期、线程模型摘要、运行控制
  (取消/超时/续跑)、会话治理、成本三层、多租户装配 cookbook、观测集成、
  错误处理边界、优雅关停顺序与策略治理回路——P2「嵌入指南」项落地;
- api.md 新增 §11「API 稳定性」:semver 纪律与稳定分层(事实冻结层 /
  稳定层 / 演进层),1.0 前破坏性变更须在 CHANGELOG 显式列出;
- 导出符号审计:全部包导出面过目一致;伞形包补导 `policy-from-agent`
  (此前导入未导出)。

### 新增
- **废弃机制(1.0 冻结的前置配套)**:符号走「稳定 → 废弃 → 移除」,
  废弃期至少保留两个次版本(紧急安全修复除外)。
  - `deprecated-warning` 警告条件(:name/:since/:use/:removed-in,
    可按类型静音);
  - `deprecate` 顶层宏:登记元数据(`deprecated-symbols` 可查,当前为空)
    并定义 compiler macro——依赖方**编译调用点**即收到警告;
  - `note-deprecated` 运行期信号:覆盖 FUNCALL/高阶传参等编译期不可见
    路径;
  - 放在 base 层(chariot-util),各层导出符号的废弃均可使用;
    登记表加载期写入、运行期只读,无锁安全;
  - api.md §11 增补「废弃流程」;新增 `deprecation-suite`
    (6 项测试,25 项断言);
  - 版本保持 0.9.0(按维护者要求:不随意提升至 1.0)。

## [0.9.0] - 2026-09-21

**主题:可观测与规模——运行标识贯穿、会话规模管理、策略与评测。**
P1 三项落地:事件/记录/嵌套统一携带运行标识(run-id);会话文件按运行边界轮转归档 + 跨会话索引;提示词/预算/采样成为带指纹的策略工件,评测跑批使调提示词可回归。

### 新增
- **运行标识(run-id)贯穿**:每次 `run` 生成唯一标识,该运行的**全部事件**
  与**全部会话记录**统一携带——宿主的日志、计费与审计无需状态跟踪即可
  按运行对齐(P1 可观测的第 4 项)。
  - 事件流:`emit-event` 单一收口盖章,每个事件 plist 携带 `:run-id`
    (嵌套运行另带 `:parent-run-id`,含流式增量);回放 `session-record->event`
    还原的事件与实时形状对称;
  - 文件侧:`session-record` 单一收口盖章,运行中写入的每条记录
    (消息/用量/meta/事件镜像)带 `run_id`(嵌套另带 `parent_run_id`);
    直接落盘形态不带;
  - 结果对象:RUN-RESULT 新增 `run-id` 槽与 `result-run-id` 访问器;
  - 取数:`session-filter` 新增 `:run-id` 参数(按运行精确圈定记录);
    新增 `session-runs`(按运行汇总:标识/父标识/模型/停止原因/轮数/
    该运行累计用量);新增 `session-record-run-id`;
  - 传播:嵌套运行(子智能体)生成新标识并继承外层为父标识,复用取消
...
- **协作式取消与运行超时**(生产控制面):`run` 新增 `:cancel-token` 与
  ...
  置位、幂等、NIL 安全),主循环在每轮开始前与每批工具执行前检查,
  以 `:cancelled` / `:timeout` 停止原因收场(非条件,消息保留可续跑);
  已计划未执行的同批工具编码为「[已取消]」失败结果;新事件 `:cancel`
  (`:reason` ∈ `:requested`/`:timeout`)随事件流镜像入会话日志。
  - 协作式语义:阻塞中的模型调用与工具执行**不打断**,分别以 Provider
    超时与工具自身超时为上界——不使用实现特定的线程中断,SBCL/CCL
    行为一致;
  - **嵌套继承**:子智能体等内层 `run` 未显式给出时,自动继承外层的
    令牌与墙钟期限(取较早者);只读工具并行执行的工作线程经词法捕获
    获得同样的取消上下文;
...
- **并发契约(有承诺、被测试)**:新增 `concurrency-suite`(13 项测试,
  ...
  - 会话落盘线程安全:`session-record` 经全局写锁完成序号分配与单行
    追加(行完整、序号唯一);「一个会话文件同一时间单写者」为契约层
...
### 新增
- **工具并行执行(整轮只读时)**:`execute-tool-calls` 重构为
  「计划 → 执行 → 收尾」三段——审批与 `:tool-call` 事件在计划期顺序
  完成,执行期在工作线程并行(仅当整轮全部只读且
  `:parallel-tools` 开启,默认开启),结果事件与消息落盘在收尾期按
  调用顺序进行。只读工具承诺无副作用,并行因此只影响墙钟时间:
  事件流、工具消息顺序与会话落盘(仅主线程收尾期发生)完全确定;
  任一变更类工具在场则整轮顺序(读-写交错风险不予接受)。
  `:parallel-tools` NIL 可恢复全顺序;`config-digest` 新增
  `parallel_tools` 字段。
