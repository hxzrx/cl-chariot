...
| `deploy/cl-chariot-mcp.service` | systemd 单元示例 |
...
1. **上传代码**到服务器(如 `/opt/cl-chariot-mcp/`),建虚拟环境装依赖:
   ```bash
   cd /opt/cl-chariot-mcp && python3 -m venv venv
   venv/bin/pip install -r requirements.txt
   ```
2. **生成鉴权 token**(公网部署必须;泄露后换一个重启即可):
   ```bash
   sudo sh -c 'cat > /etc/cl-chariot-mcp.env <<EOF
   MCP_BEARER_TOKEN=$(openssl rand -hex 32)
   MCP_ALLOWED_HOSTS=cantos.cn
   EOF
   chmod 600 /etc/cl-chariot-mcp.env'
   ```
...
3. **systemd 托管**:按 `deploy/cl-chariot-mcp.service` 内注释安装
   (替换 `<运行用户>` 占位),`enable --now` 后确认 `systemctl status`
   与 `journalctl -u cl-chariot-mcp` 正常。服务器只监听 `127.0.0.1:8765`。
...
安全清单:token 只放 `/etc/cl-chariot-mcp.env`(600);本目录所有工具都是
无害测试桩,但**请勿**在公网无鉴权运行;如需更严可加 nginx 限流与 IP 白名单;
token 泄露时换值重启 systemd 单元即可。

## 与 cl-chariot 联调

### 现在(stdio 客户端 + mcp-remote 桥接,cl-chariot 无需改动)

cl-chariot 当前实现的是 stdio 传输;经社区的 stdio↔HTTP 桥接器即可连上:

```lisp
(let ((client (chariot-mcp:make-mcp-client "npx" "-y" "mcp-remote"
                                        "https://cantos.cn/mcp"
                                        "--header" "Authorization: Bearer <token>")))
  (chariot-mcp:initialize client)
  (let ((tools (chariot-mcp:mcp-tools-from-server client)))
    ;; tools 可直接传入 (chariot-agent:make-agent :tools ...) 使用
    ...))
```
...
# 未设置 CHARIOT_MCP_URL 时该套件自动跳过,不影响离线全量
```

### 原生直连(cl-chariot 已内置 Streamable HTTP 传输)
...
- **协议协商**:cl-chariot 客户端现声明 `2025-11-25`,本服务器(SDK 1.30,
...
- `smoke_test.py` 与 cl-chariot 无关,任何符合 MCP 的客户端实现都可以拿它
  做部署验收。
