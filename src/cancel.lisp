;;;; cancel.lisp —— CL-Chariot 运行上下文:取消令牌、墙钟期限与运行标识
;;;;
...
;;;;
;;;; 三类运行上下文经动态变量向嵌套运行传播:取消令牌、墙钟期限与运行
;;;; 标识。子智能体等内层 RUN 未显式给出 :CANCEL-TOKEN / :TIMEOUT 时继承
;;;; 外层绑定——取消外层运行同样止住内层工作,外层的时间预算也约束内层
;;;; (期限取较早者);运行标识则内层生成新值、外层值保留为父标识。
...
(defparameter *run-id* nil
  "当前运行的唯一标识(动态变量;RUN 开始时经 GEN-ID 生成并绑定)。
事件流(EMIT-EVENT)与全部会话记录(SESSION-RECORD)据此统一加盖运行
标识——宿主的日志、计费与审计无需状态跟踪即可对齐。嵌套运行重新
绑定新值,外层值经 *PARENT-RUN-ID* 保留。")

(defparameter *parent-run-id* nil
  "嵌套运行的外层运行标识:内层 RUN 启动时记录外层 *RUN-ID*。
顶层运行为 NIL;事件与会话记录据此体现归属链。")
