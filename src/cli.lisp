...
REPL 斜杠命令:
  /help /quit /tools /provider NAME /model NAME /usage /clear /system TEXT
")

(defun parse-args (argv)
  "解析命令行参数为 plist:
(:prompt STR :provider SYM :model STR :api-key STR :base-url STR :system STR
  :tools STR :max-turns N :permission SYM :stream BOOL :color BOOL
  :session STR :temperature X :help BOOL :version BOOL :list-providers BOOL)"
  (let ((opts '())
        (positional '()))
    (labels ((next (i)
...
(defun run-oneshot (opts)
  ...
  (let ((agent (apply #'clh-agent:make-agent (opts->agent-args opts))))
    (handler-case
...
局部变量中——CLI 层的必要可变状态,不进入库层。"
  (let* ((args (opts->agent-args opts))
         (provider (getf args :provider))
         (system-prompt (getf opts :system))
         (conversation '())            ; 续跑消息(不含 system,system 由 agent 注入)
         (last-usage nil))
    (print-banner provider)
    (loop
      (fresh-line)
      (princ (cyan "› "))
      (finish-output)
...
(defun handle-slash-command (line &key provider-ref system-ref conversation-ref usage-ref usage-set)
  ...
  (let* ((trimmed (clh-util:trim-whitespace line))
    ...
    (cond
      ((member cmd '("/help" "/?") :test #'string=)
       (format t "~A~%" (dim "命令:/quit 退出 /tools 工具列表 /provider NAME 切换厂商
/model NAME 切换模型 /usage 上次用量 /clear 清空对话 /system TEXT 设置系统提示词")))
      ((string= cmd "/quit") (uiop:quit 0))
      ((string= cmd "/tools")
