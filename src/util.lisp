...
;;; 非加密散列(配置摘要)
;;;;
;;; FNV-1a 64:不抗碰撞攻击,只用于配置摘要、变更检测与对账;
;;; 逐字符按 char-code(码点,非 UTF-8 字节)散列,显式按 2^64 截断,
;;; 任意 ANSI 实现结果一致。
;;; ---------------------------------------------------------------------------

(defconstant +fnv-1a-64-prime+ #x100000001b3 "FNV-1a 64 位素数。")
(defconstant +fnv-1a-64-basis+ #xcbf29ce484222325 "FNV-1a 64 位偏移基底。")

(defun fnv-1a-hex (&rest strings)
  "对 STRINGS 依次拼接做 FNV-1a 64 位散列,返回 16 位小写十六进制字符串。
非加密散列,仅用于配置摘要/变更检测;空输入返回偏移基底(与规范一致)。"
  (let ((hash +fnv-1a-64-basis+))
    (dolist (s strings)
