...
| 15 | 轮转归档 = 复制 + 原子重写;保留段序号重排,跨文件关联键为 run_id | 写入器按记录数续序号,稠密重排保证轮转后续跑不回绕不碰撞;原始序号保存在归档副本,run_id 是跨文件的全局关联键(seq 各文件独立自洽);重写走同目录临时文件 + 改名覆盖,崩溃无半写状态 |
| 16 | 策略(提示词/预算/采样)与接线(provider/工具/回调)分离为 POLICY 工件;评测以策略指纹归集 | 调提示词成为可版本化、可回归的工程行为,而非改代码;「自动晋升只许动提示词/预算」的权限边界有了承载物;评测行同时携带策略指纹与 run_id,配置差异与运行上下文都可对账 |
